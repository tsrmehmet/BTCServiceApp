function TreeView(datas, options) {
    ...
    function setCheckState(value) {
        var selectedID = $(this).find(">[data-id]")[0];
        this.checked = value
    ...
    function createSingleItem(data) {
        ...
        if ("children" in data && data.children!=null && data.children.length > 0) {
            item.onclick = groupToggle.bind(item);
        } else { item.onclick = changeCheckState.bind(item); }
    ...
    this.update = function () {
        $(t.root).find(".group").each(function (index, el) {
            if ($(el).find(".group").length > 0) {
                ...
                $children = $(el).find(".item");
                $($children).find(">[check-icon]").css("visibility", "hidden");

            } else {
                $(el).find(">[fold-button]").css("visibility", "hidden");

                $children = $(el).find(".item");
                $($children).find(">[check-icon]").css("visibility", "visible");
            }
